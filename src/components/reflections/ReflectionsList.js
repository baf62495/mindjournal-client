...
export default class LogsList extends React.Component {
  render() {
    ...
    const list = this.props.reflections
      ...
      .map((reflection, key) => (
        <ReflectionsListItem {...reflection} key={key} />
      ));
    if (reflections.length === 0) {
      return (
        <div className='mj-refl-list'>
          <p>
            <i>Create your first reflection...</i>
          </p>
        </div>
      );
      ...
      return (
        <div className='mj-refl-list'>
          <p>
            <i>Sorry, there are no reflections that match the criteria</i>
          </p>
        </div>
      );
    }
    return <div className='mj-refl-list'>{list}</div>;
  }
}
