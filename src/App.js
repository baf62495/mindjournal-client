...
class App extends React.Component {
  ...
  componentDidMount() {
    Promise.all([
      ...
      .then(([logsRes, reflectionsRes]) => {
        if (!logsRes.ok) return logsRes.json().then(e => Promise.reject(e));
        if (!reflectionsRes.ok)
          return reflectionsRes.json().then(e => Promise.reject(e));
        return Promise.all([logsRes.json(), reflectionsRes.json()]);
      })
  ...
  createReflection = e => {
    ...
    fetch(`${config.API_BASE_URL}/reflections`, {
      ...
      .then(reflection => {
        this.setState({
          reflections: [...this.state.reflections, reflection]
        });
      });
  };
      ...
      <>
        ...
        <Route
          ...
          render={rprops => (
            <ReflectionsFormPage
              {...rprops}
              createReflection={this.createReflection}
            />
          )}
        />
      </>
